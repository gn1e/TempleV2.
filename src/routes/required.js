...
export default function () {
    ...
    app.get('/waitingroom/api/waitingroom', (c) => {
        return c.body('thank for using templebakent!', 204, {
            'X-Message': 'nice person',
    ...
    app.post("/links/api/fn/mnemonic", async (c) => {
        const MnemonicArray = discovery.v2.Panels[1].Pages[0].results.map(result => result.linkData);
    ...
    app.get("/links/api/fn/mnemonic/*", async (c) => {
        ...
        const result = discovery.v2.Panels[1].Pages[0].results.find(result => result.linkData.mnemonic === mnemonic);
        if (result) {
